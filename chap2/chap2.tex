...
There is no separation between a {\itshape brain program} and
...
\smplenquote{the red one} to identify the red square as the topic. 
If the context contains also a red triangle, he has to be more 
precise and say something like \smplenquote{the red square} to delineate
the topic from the red triangle as well as
from the blue square. 
Of course, the Talking Heads do not say \smplenquote{the red square}
but use their own language and concepts which are not 
necessarily the same as those used in English. For example, 
they may say \smplenquote{malewina} to mean [\textsc{upper extreme}-\textsc{left low}-\textsc{redness}]. 
...
at the dining table and say \smplenquote{could you give me the salt}. 
...
be close to the salt and my pronunciation of the word \smplenquote{salt}
may have sounded a bit like \smplenquote{salmon}, perhaps because there was
...
either. We say \smplenquote{could you give me the red small square} as 
opposed to \smplenquote{could you give me O\_143}. Natural language
words like \smplenquote{red} or \smplenquote{small} name perceptually grounded categories
...
\begin{itemize}
\item An {\itshape utterance}, such as \smplenquote{small red square}, which is 
...
For example, he might choose \smplenquote{blauw} (if he speaks
Dutch), or \smplenquote{bleu} (if he speaks French),
...
like \smplenquote{wabido}, \smplenquote{bimaku}, etc. The articulation and 
...
Every physical Talking Heads set-up features a {\itshape waiting room} in 
...
\smplenquote{gofubo} has been used to successfully refer to them.}
\label{f:plate10}
\end{figure}
...
relating [\textsc{vpos} 0.0–0.5]$_{\mathbf  a1}$ with the word \smplenquote{lu}, 
...
association between \smplenquote{lu} and [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$, and so
hypothesises that [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$ must be the meaning of \smplenquote{lu}. 
...
{\bfshape a1} might have created the word \smplenquote{lu}, and associated it 
...
form heard, \smplenquote{lu}, and the guessed meaning [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$. 
Based on this extended lexicon, he will succeed in the same game 
in the future and he can use \smplenquote{lu} himself to verbalise [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$
...
another agent had already used the word \smplenquote{lu} 
for another meaning, so that there are now 
two alternative meanings for \smplenquote{lu} in the lexicon. \smplenquote{lu} 
has become ambiguous. Second, it may be the hearer {\bfshape a2} already 
had a word for [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$, for example \smplenquote{bomida}, 
...
process unfolds with periods where one word (first \smplenquote{xu} and then \smplenquote{fepi}) dominates.} 
\end{figure}
...
hearer could just as well have hypothesised that \smplenquote{lu} means [\textsc{hpos} 0.0–0.5] (left)
...
same scene with {\bfshape a1} saying \smplenquote{lu} to mean [\textsc{vpos} 0.0–0.5], and 
{\bfshape a2} interpreting \smplenquote{lu} as [\textsc{hpos} 0.0–0.5], the game would succeed! 
...
success despite the different meanings of \smplenquote{lu}, 
and the players would never discover
that each means something else by \smplenquote{lu}. 
...
\smplenquote{stop for the red light} does not mean stop for 
...
can be used to disambiguate \smplenquote{lu}.}
\end{figure}
...
hearer {\bfshape a2} has associated \smplenquote{lu} with [\textsc{hpos} 0.0–0.5]$_{\mathbf  a2}$, he
would interpret \smplenquote{lu} as [\textsc{hpos} 0.0–0.5]$_{\mathbf  a2}$ (to the left). But this 
does not yield a single 
referent and therefore the game fails. 
This failure is an opportunity for the hearer to repair his 
hypotheses about the possible meanings of \smplenquote{lu}.
...
\smplenquote{lu} and [\textsc{vpos} 0.0–0.5]$_{\mathbf  a2}$ is stored. The old association
...
dominating meaning of \smplenquote{lu} can still go in 
two directions. For example, {\bfshape a2}
could have used \smplenquote{lu} with {\bfshape a1} 
...
\smplenquote{lu} and [\textsc{hpos} 0.0–0.5]. If this happened often 
enough [\textsc{hpos} 0.0–0.5] (left) might
have become the dominant meaning of \smplenquote{lu}, instead of 
...
For example, we say \smplenquote{the triangle to the left of the square}, in 
other words left with respect to the square, to avoid the uncertainty
inherent in the absolute use of \smplenquote{left}. Humans scale the size 
