...
and meaning can evolve and be acquired ab initio\todo{GSR: no italics in commonly used loan words}. 
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=.85\textwidth]{chap2/figs/triangle2.pdf}}
...
context to find the referent \tabref{tab:2:findthereferent}\todo{maybe: see Table 2.1??}. 
...
\end{tabular}
\end{table}
\todo[inline]{This table needs a caption}
...
and would therefore be categorised as [LEFT]. Similarly, the 
...
\figref{scene1-1} would be categorised as [RIGHT UPPER LIGHT].
...
bottom region of the HPOS-channel (categorised as 
[LEFT]) into two subregions [TOTALLY-LEFT] ($0.0 \le HPOS < 0.25$), 
and [MID-LEFT] ($0.25 \le HPOS < 0.5$). The triangular object
can now be categorised as [MID-LEFT], if simply categorising it
...
of vision ($0.0 \le $HPOS$ < 0.5$) is categorised as
left, i.e. [HPOS 0.0 0.5], and everything right  
($0.5 < $HPOS$ \le 1.0$) is categorised as right, 
