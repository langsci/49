...
when a speaker says \smplenquote{wabo} and the hearer has correctly pointed 
...
language might mean when he utters \smplenquote{gavagai} while pointing
...
like \smplenquote{coba} or \smplenquote{wabidu}, for forming words, and that they 
...
word \smplenquote{pama} for [\textsc{dark}] and \smplenquote{limiri} for [\textsc{large}]. 
...
highest score (\smplenquote{pama}), and transmits that form. 
...
\smplenquote{di}. The hearer receives the word, does not know it,
...
a new word \smplenquote{pida} for [\textsc{large}]. {\bfshape a3} does not know 
...
\smplenquote{pida} for [\textsc{large}]. {\bfshape a3} hears \smplenquote{pida}, has associated it 
...
\end{center}
\end{table}
Both the words \smplenquote{di} and \smplenquote{pida} are present but with 
weak scores. There are two synonyms for 
{}[\textsc{large}]: \smplenquote{pida} and \smplenquote{fobu}, but \smplenquote{pida} is 
preferred. There is a word \smplenquote{gi} who is available 
...
\smplenquote{pida}. 60 \% of the agents
use \smplenquote{gi} for [\textsc{light}], and 40 \% \smplenquote{di} for [\textsc{small}]. 
...
{\bfshape a5}'s score for \smplenquote{gi} is 0.0
...
games showing how \smplenquote{ga} propagates from {\bfshape a4} to {\bfshape a2} in 
...
synonyms may occur.  
A speaker of British English typically uses the word \smplenquote{pavement}, 
whereas an American prefers \smplenquote{sidewalk}, even though he
understands \smplenquote{pavement}. Thus there can 
...
one form (\smplenquote{pe}) emerges as the winner. When we later study 
...
with one word (\smplenquote{pe}) dominating.}
\end{figure}
...
\end{verbatim}
Some words (for example \smplenquote{tine} for [M5] or \smplenquote{go} for [M2]) are
...
the word \smplenquote{mugu} for [M19] is preferred for the first object in the first
cluster and known but not preferentially used by the agent in the second cluster.
Conversely, \smplenquote{ninu} is preferred for the same meaning by the agent in the second
cluster, although he also knows \smplenquote{mugu}.
\smplenquote{mepabo} for [M7] belongs to the interlingua. Both agents
know and use it but they have a strong alternative \smplenquote{jabeto} for 
the first and \smplenquote{junipe} for the second agent. 
