...
learned words about objects in the environment, such as \smplenquote{ball} or \smplenquote{smiley}, as well as words for actions, such as 
\smplenquote{sit down} or \smplenquote{stand up}. 
...
perspective of the hearer. For example, the speaker might say \smplenquote{the ball rolls from my left to your right}.}
\end{figure}
...
distance (see \figref{fig:tokyo-persp}). The two robots next play a description game. One robot (acting as the \smplenquote{speaker}) 
...
reversal (as in \smplenquote{{\itshape to your} left} vs. simply \smplenquote{left}). Only with this strategy are agents able to reach high 
...
\end{figure}
An application of these schemas in producing the utterance \smplenquote{(W O) (W U)} is represented in \figref{fig:associations} and shown 
here: 
\begin{verbatim}
...
In the sentence \smplenquote{John gave Mary a book}, John plays the role of agent, the giver 
...
\smplenquote{Mary gave John a book} contains the same participants as \smplenquote{John gave Mary a book} but now Mary is the 
agent and John the recipient. Still other languages, like Japanese, use a strategy based on particles following 
the noun, as in the Japanese sentence \smplenquote{Tanaka-san wa Tokyo de o-to-san ni atta} 
meaning Tanaka met his father in Tokyo. Tanaka-san is the name of a person, oto-san means \smplenquote{father} and atta 
means \smplenquote{met}. The markers are \smplenquote{wa} for the topic, \smplenquote{de} for location and \smplenquote{ni} for another sense of location which can 
...
objects (such as \smplenquote{smooth}, \smplenquote{red}, etc.)
as well as actions involving objects (such as \smplenquote{move-away-from}, \smplenquote{move-towards} or \smplenquote{push}). English words
...
like \smplenquote{move-towards red jill} (Jill moves towards the red object) or `push 
...
event. For example, there could be two markers: \smplenquote{pi} 
and \smplenquote{pa} specifying that the referent plays the role respectively of giver and gift in a give event, as in
\smplenquote{pi jill pa block give}. The markers are entirely specific for each event in the sense that there is no generalisation 
...
the utterance \smplenquote{po smooth move-away-from}. Syntactic properties such as the form of the word, 
...
Here is an example of a lexical rule that defines the meaning of the word for an event \smplenquote{move-away-from}. The referent is 
...
((?unit
   (form (== move-away-from)))))
\end{verbatim}
Next, here is an example of a rule for the marker \smplenquote{po}. On the semantic side it ensures that the referent of one unit 
whose referent is ?source is equal to the object filling the argument move-away-from-2 in a move-away-from event. 
On the syntactic side, it triggers when there is a word that has the grammatical category \smplenquote{takes-marker-1} and 
is preceded by the marker \smplenquote{po}. (<< stands for \smplenquote{precedes}) 
\begin{verbatim}
((?predicate 
   (meaning
    (== (move-away-from ?evnt)
...
grammatical category. For example, the word \smplenquote{smooth} has the category \smplenquote{takes-marker-1}: 
\begin{verbatim}
((?unit
   (gramcat (== takes-marker-1)) 
   (form (== smooth))))
...
The marker \smplenquote{po} in the above example is entirely specific for one argument of the move-away-from predicate, as would be the 
