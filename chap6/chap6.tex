...
of the square to find the word \smplenquote{pokuneso} for this chosen meaning. 
The hearer traverses the lexical side of the triangle in 
the other direction to interpret the word \smplenquote{pokuneso} as
...
agents associate the word \smplenquote{okuneso} with 
...
puzzled by the word \smplenquote{gavagai} uttered by a native 
in an undecoded language. Does \smplenquote{gavagai} mean rabit, animal 
...
\smplenquote{bowaluro}, and uses it. The hearer {\bfshape  a1} 
does not know the 
...
Game 77
  ...
  a2 says: \smplenquote{bowaluro}
  a1 does not know \smplenquote{bowaluro}
  a1 says: \smplenquote{bowaluro?}
  a2 points to rectangle-1
  a1 categorises the topic as [HEIGHT 0.75–1.0]
  a1 stores \smplenquote{bowaluro} as [HEIGHT 0.75–1.0]
\end{verbatim}
The reason why {\bfshape  a1} has guessed the right 
meaning of \smplenquote{bowaluro} is because both agents use
...
Game 96
  ...
  a1 creates a new word: \smplenquote{kutaki}
  a1 says: \smplenquote{kutaki}
  a2 does not find a unique referent
  a2 says: \smplenquote{kutaki?}
  a1 points to rectangle-1
  a2 categorises the topic as [WIDTH 0.0–0.25]
  a2 stores \smplenquote{kutaki} as  [WIDTH 0.0–0.25]
\end{verbatim}
...
running for [\textsc{hpos} 0.5–1.0]: \smplenquote{rutaxese} and \smplenquote{xomupovi}, and 
three for [\textsc{vpos} 0.5–1.0]: \smplenquote{wavone}, \smplenquote{zaxawe}, and 
\smplenquote{dazofo}. Some words are already well established, for
example \smplenquote{numefuli} for [\textsc{gray} 0.5–1.0] (dark). 
...
with one word \smplenquote{faluleru} being the winner. 
For others, like [\textsc{hpos} 0.5–1.0], the competition 
is still going on, although we can guess
that \smplenquote{xomupovi} is probably going to be the winner. 
...
\smplenquote{faluleru} is the first one that has some success. 
\begin{verbatim}
Game 76
 Speaker a4 creates \smplenquote{notabefe} for [GRAY 0.5–1.0]
 Hearer a5 adopts \smplenquote{notabefe} for [GRAY 0.5–1.0]
Game 79 
 Speaker a2 creates \smplenquote{vivevobo} for [GRAY 0.5–1.0]
 Hearer a7 does not adopt \smplenquote{vivebo} 
     (failed to discriminate)
Game 88 
 Speaker a7 creates \smplenquote{faluleru} for [GRAY 0.5–1.0]
 Hearer a4 adopts \smplenquote{faluleru} for [GRAY 0.5–1.0]
Game 100 
 Speaker a7 uses \smplenquote{faluleru} for [GRAY 0.5–1.0]
 Hearer a4 correctly interprets \smplenquote{faluleru}
\end{verbatim}
...
\smplenquote{notabefe} and picked up \smplenquote{faluleru} from {\bfshape  a7}. 
...
{}[\textsc{gray} 0.5–1.0]. {\bfshape  a4} chooses \smplenquote{faluleru} because this 
...
Game 101
  ...
  a4 has two words for [GRAY 0.5–1.0]:
       \smplenquote{faluleru} (0.20)
       \smplenquote{notabefe} (0.00)
  a4 says: \smplenquote{faluleru}
  a1 does not know \smplenquote{faluleru}
  a1 says: \smplenquote{faluleru?}
  a4 points to rectangle-1
  a1 categorises the topic as [GRAY 0.5–1.0]
  a1 stores \smplenquote{faluleru} as [GRAY 0.5–1.0]
\end{verbatim}
After this game, three agents \smplenquote{know} the word 
\smplenquote{faluleru} for [\textsc{gray} 0.5–1.0]: {\bfshape  a4}, {\bfshape  a1}, 
and {\bfshape  a7}. When we continue to inspect the
simulation we see that \smplenquote{notabefe} takes a bit of 
a revenge. The next games with the 
meaning [\textsc{gray} 0.5–1.0] all involve the word \smplenquote{notabefe}: 

\begin{verbatim}
Game 111
 Speaker a5 uses \smplenquote{notabefe} for [GRAY 0.5–1.0]
 Hearer a4 correctly interprets \smplenquote{notabefe}
Game 113
 Speaker a4 uses \smplenquote{notabefe} for [GRAY 0.5–1.0]
 Hearer a9 adopts \smplenquote{notabefe} for [GRAY 0.5–1.0]
Game 127
 Speaker a9 uses \smplenquote{notabefe} for [GRAY 0.5–1.0]
 Hearer a6 adopts \smplenquote{notabefe} for [GRAY 0.5–1.0]
\end{verbatim}
But then there is another occurrence of \smplenquote{faluleru}: 
\begin{verbatim}
Game 135 
 Speaker a1 uses \smplenquote{faluleru} for [GRAY 0.5–1.0]
 Hearer a2 adopts \smplenquote{faluleru} for [GRAY 0.5–1.0]
\end{verbatim}
...
creating a new word, \smplenquote{sopine}, which {\bfshape  a5}, playing
the role of hearer, adopts. 
\begin{verbatim}
Game 140
 Speaker a6 creates \smplenquote{sopine} for [GRAY 0.5–1.0]
 Hearer a5 adopts \smplenquote{sopine} for [GRAY 0.5–1.0]
\end{verbatim}
This kind of evolution continues with a struggle between 
\smplenquote{notabefe} and \smplenquote{faluleru}. The agents know both 
words so the games do not fail. But \smplenquote{faluleru}, 
just by chance, starts to occur a bit more often
which causes its score to go up a bit more. 
This results in \smplenquote{faluleru} being used even more, 
and, due to lateral inhibition, \smplenquote{notabefe} used 
less. Gradually \smplenquote{faluleru} dominates for 
the whole population. 
After 2000 games, the competitors to \smplenquote{faluleru} have 
...
Game 3
  ...
  a1 says: \smplenquote{mibati}
  a2 does not know \smplenquote{mibati}
  a2 says: \smplenquote{mibati?}
  a1 points to rectangle-1
  a1 considers as salient AREA WIDTH HEIGHT HPOS 
  a1 categorises the topic as [HEIGHT 0.0–0.5]
  a1 stores \smplenquote{mibati} as [HEIGHT 0.0–0.5]
\end{verbatim}
...
In the next game of the simulation series, \smplenquote{mibati}
...
{}[GRAY 0.5–1.0]
  ...
  a2 says: \smplenquote{mibati}
  a1 interprets \smplenquote{mibati} as [HEIGHT 0.0–0.5]
  a1 points to rectangle-1
  a2 signals OK 
\end{verbatim}
...
a word for it \smplenquote{puxazi}, and transmits this to the hearer. 
...
Game 9
  ...
  a2 creates a new word: \smplenquote{puxazi}
  a1 says: \smplenquote{puxazi}
  a2 does not know \smplenquote{puxazi}
  a2 says: \smplenquote{puxazi?}
  a1 points to square-1
  a2 considers as salient GRAY WIDTH VPOS HPOS
  a2 categorises the topic as [GRAY 0.0–0.5]
  a2 stores \smplenquote{puxazi} as [GRAY 0.0–0.5]
\end{verbatim}
...
\smplenquote{puxazi} and if the meanings are compatible, they 
...
Game 11
  ...
  a2 categorises the topic as [GRAY 0.0–0.5]
  a2 says: \smplenquote{puxazi}
  a1 interprets \smplenquote{puxazi} as [VPOS 0.5–1.0]
  a1 points to rectangle-1
  a2 signals OK 
\end{verbatim}
...
adopts yet another meaning for \smplenquote{puxazi}, namely 
{}[\textsc{height} 0.5–1.0].}
\end{figure}
Here is next a game (game 14) where {\bfshape  a2} comes to adopt 
the other meaning of \smplenquote{puxazi}, because the first
...
{}[\textsc{vpos} 0.5–1.0], which he has lexicalised as \smplenquote{puxazi}. 
For {\bfshape  a2}, \smplenquote{puxazi} means [\textsc{gray} 0.0–0.5] (light), but this 
...
{}[VPOS 0.5–1.0] [HEIGHT 0.5–1.0]
  a1 says: \smplenquote{puxazi}
  a2 interprets \smplenquote{puxazi} as [GRAY 0.0–0.5]
  ...
  a2 stores \smplenquote{puxazi} as [HEIGHT 0.5–1.0]
\end{verbatim}
...
guarantee that \smplenquote{puxazi} would have been associated 
with [\textsc{height} 0.5–1.0] by {\bfshape  a2}. Moreover {\bfshape  a2} stores
an association between \smplenquote{puxazi} and [\textsc{height} 0.5–1.0], even though 
...
\smplenquote{puxazi} is increased in both agents. The relation between \smplenquote{puxazi} and 
...
identify the referent: \smplenquote{puxazi} (meaning 
{}[\textsc{gray} 0.0–0.5], i.e. light) and \smplenquote{torigusu}
(meaning [\textsc{vpos} 0.0–0.5], i.e. lower).  \smplenquote{puxazi} has
a higher score and wins the competition. Because the game 
was successful, the score
of \smplenquote{puxazi} goes up. The score of \smplenquote{torigusu} 
does not change because it concerns another 
meaning. Two meanings for \smplenquote{puxazi}
...
further disambiguate \smplenquote{puxazi}. 
...
{}[GRAY 0.0–0.5] [GRAY 0.0–0.25]
  ...
  a2 says: \smplenquote{puxazi}
  a1 interprets \smplenquote{puxazi} as
{}[GRAY 0.0–0.5] (0.20)
{}[VPOS 0.0–0.5] (0.0)
  a1 points to rectangle-1
  a2 signals OK
...
and \smplenquote{puxazi} will have a score of 0.3 in both agents. 
The association between \smplenquote{puxazi} and [\textsc{vpos} 0.0–0.5] 
...
Note that there are two meanings for \smplenquote{puxazi}: \enlargethispage{1\baselineskip}
...
The second meaning of \smplenquote{puxazi} has now disappeared
...
\textsc{area} and \textsc{height}: \smplenquote{texiraxi} and \smplenquote{wixizode}
...
\end{figure}
For the word \smplenquote{texiraxi}, the scores of the different 
...
\lsptoprule
{\itshape Agent} & 
\multicolumn{2}{c }{\itshape Scores \smplenquote{texiraxi}} &
\multicolumn{2}{c }{\itshape Scores \smplenquote{wixizode}}\\ 
...
{}[HEIGHT 0.5–1.0] [HEIGHT 0.75–1.0] 
  ...
  a2 says: \smplenquote{wixizode}
  a7 interprets \smplenquote{wixizode} as
{}[AREA 0.5–1.0] (1.0)
{}[WIDTH 0.0–0.5] (0.0)
{}[HEIGHT 0.5–1.0] (0.0)
...
tall and large. French has one word \smplenquote{grand} encapsulating
...
\smplenquote{kodawika} and \smplenquote{togixa} for [\textsc{gray} 0.5.0.75], or 
\smplenquote{vavuvosi} and \smplenquote{radude} for [\textsc{width} 0.75–1.0]. See \tabref{tab:comp}. 
...
for \smplenquote{sakezomo} and \smplenquote{nopofi}). But these words are 
very short-lived. The new agent has already picked up 
\smplenquote{larubo} which is the word in use expressing the 
meaning of \smplenquote{sakezomo}. \smplenquote{larubo} has already a higher 
score so \smplenquote{sakezomo} will definitely disappear. 

The most widespread words in the lexicon 
such as \smplenquote{tituroxu} or \smplenquote{legoka} are the ones that 
...
has solidly associated the word \smplenquote{texiraxi} with 
{}[\textsc{height} 0.5–1.0] and \smplenquote{wixizode} with [\textsc{area} 0.5–1.0]
...
\smplenquote{fumetese} has been associated with [\textsc{width} 0.0–0.5] 
