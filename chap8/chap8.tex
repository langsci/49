...
Bruno Verbergt of an organisation called \smplenquote{Antwerpen Open}.
...
also opportunities to use multiple words (such as \smplenquote{red bottom}). 
...
there are other possibilities. There are three competing words for naming blue: \smplenquote{Xagadude}, \smplenquote{Nibidesu}, and \smplenquote{Tetipi}. 
None of them have a score higher than 0 and so a random choice of \smplenquote{Xagadude} is made. 
...
recognised by the hearer using the word \smplenquote{kazozo}. The meaning of the word was not visible through the interface because we 
...
set of objects and the meaning of \smplenquote{kazozo} was effective in finding the same topic. 
}
\end{figure}
...
meanings for a particular word, namely \smplenquote{droite} (meaning \smplenquote{to the right} in French). This word clearly has been introduced
...
\caption{\label{fig:droite}Form-Meaning diagram: Different meanings associated with the word \smplenquote{droite}. The dominant meaning is 
...
\caption{\label{fig:bozopite}Form-Meaning diagram: Different meanings associated with the word \smplenquote{bozopite}. 
}
\end{figure}
...
frequency of meanings for the word \smplenquote{down}. The meanings are all concepts on the vertical position channel 
...
the art context.\footnote{Examples are a \smplenquote{gallery talk} as Salon 3 at the Elephant and Castle Centre
